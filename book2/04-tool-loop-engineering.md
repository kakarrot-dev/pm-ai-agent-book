...
一次性把全部 Tool Schema 放进 Context，会同时增加 token、选择干扰和权限暴露。工具越多，模型越容易在名称相近的动作之间选错，也更难解释某个高风险工具为什么出现在当前任务中。

渐进式披露把工具信息分成三层：

| 层级 | 模型看到什么 | 作用 |
| --- | --- | --- |
| 能力目录 | 名称、用途、风险等级、所属域和发现入口 | 判断是否需要进一步查看 |
| 工具详情 | 完整 Schema、前置条件、示例、错误和版本 | 形成候选调用 |
| 执行授权 | 当前 Task、身份、资源、额度和确认结果 | 决定候选调用能否生效 |

能力目录可以来自本地注册表、Skill 元数据或 MCP 发现结果。它只说明某项能力可能可用，不等于当前用户已经获得调用权限。模型按任务需要加载工具详情，运行时再根据身份、资源、风险和任务状态生成本轮允许列表。

工具发现本身也要受控：

- 搜索结果分页并限制数量，避免一次返回整个目录；
- 发现结果带来源、版本和有效期，Schema 变化后重新评估；
- 同名或近义工具显示所属 Server、风险和数据边界；
- 高风险工具不能仅因模型搜索到就进入可调用列表；
- 记录模型看过哪些能力、为什么选中当前工具；
- 没有合适工具时允许停止或澄清，不能用名称相近的工具凑数。

Skill 可以把领域方法、工具选择规则和依赖组织在一起，但 Skill 仍然不是执行权限。加载退款 Skill 后，模型可以知道应检查政策、计算金额和请求确认；真正提交退款时，运行时仍要校验订单、额度、用户身份和具体授权。
...
Tool Loop 把一次工具调用变成连续的产品决策：模型根据当前状态按需发现能力并提出动作，系统完成参数、权限和风险校验，工具返回环境证据，运行时更新任务状态，再决定继续、等待或停止。Skill、Workflow、Function Calling 与 MCP 可以参与这条链路，但分别承担方法封装、确定控制、结构化请求和连接协议责任。
