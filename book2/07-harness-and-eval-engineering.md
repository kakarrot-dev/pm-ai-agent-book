...
同一个模型在同一道题上运行十次，偶尔答对一次，和连续十次都答对，是两种不同的产品能力。前者说明正确行为已经出现在模型的输出分布中，可以继续研究 Prompt、Context、工具或模型优化；后者才接近用户能够依赖的服务。

`Pass@k` 衡量运行 `k` 次后至少成功一次的比例，适合探测能力上限。若单次成功概率近似为 `p`，各次采样近似独立，则：

```text
Pass@k = 1 - (1 - p)^k
```

`Pass^k` 衡量连续 `k` 次全部成功的概率，近似为 `p^k`，适合观察业务可靠性。单次成功率为 90% 时，连续 10 次全部成功的概率只有约 35%。用户每周提交一次重要任务，感受到的是连续使用是否可靠，不是系统多试几次能否碰到正确结果。

这两个指标服务不同决策：

| 指标 | 回答的问题 | 适合场景 | 不能推出什么 |
| --- | --- | --- | --- |
| `Pass@k` | 模型多次尝试后是否存在成功路径？ | 模型选型、能力探索、判断是否值得继续优化 | 不能证明线上单次调用可靠 |
| `Pass^k` | 同类任务连续执行是否稳定成功？ | 业务可靠性、自动化范围、人工兜底设计 | 不能解释失败发生在哪一步 |

实际运行可能共享缓存、环境和历史状态，未必满足独立假设。Harness 应报告实测的重复运行结果、样本量和运行条件；公式用于理解指标，不代替实验。高风险任务还要同时检查副作用、授权和结果证据，连续答对文本不等于连续安全完成任务。
...
核心、边界、风险和历史回归描述的是样本内容。工程上还要按决策目标区分数据集，避免一个不断膨胀的 Dataset 同时承担防回归、探索能力和代表生产分布三种互相冲突的任务。

| 数据集 | 回答的问题 | 更新方式 | 主要风险 |
| --- | --- | --- | --- |
| 稳定回归集 | 已经修复的重要问题是否再次出现？ | 谨慎增删，保留长期可比性 | 只看旧问题，错过新能力 |
| 生产分布集 | 当前真实用户主要在做什么、失败在哪里？ | 按固定周期从脱敏 Trace 抽样 | 被流量变化和重复样本扭曲 |
| 能力探针集 | 新模型、工具或 Context 策略出现了什么能力边界？ | 每次重大版本主动设计与探索 | 容易把演示能力误当稳定价值 |
| 风险门禁集 | 哪些严重行为在任何版本都不能出现？ | 安全事件和威胁模型变化时更新 | 覆盖不足产生虚假安全感 |

同一条样本可以被引用到不同集合，但应有稳定 `example_id` 和明确来源。候选版本先通过风险门禁与稳定回归，再用生产分布集判断整体收益；能力探针用于发现机会，不能未经真实任务验证就直接成为发布依据。

#### 评估用例也需要失效和退出机制

评估集不是只增不减的档案库。以下情况应触发复核：

- 业务规则、产品范围或权威资料已经变化；
- 对应功能已经下线，用户无法再进入该路径；
- 多条样本实际描述同一个失败，导致局部分布权重过高；
- 参考答案无法说明来源或责任人；
- 评估器长期无法与人工判断一致；
- 样本已连续多个版本通过，但不再对应重要风险或用户价值。

复核结果可以是保留、更新、合并、降级为观察指标或归档。归档不等于删除历史：应记录失效原因、最后适用的产品版本和替代样本，使旧实验仍可解释。风险门禁用例即使长期通过，也不能仅因“已经解决”而自动退出。
...
一条轨迹可能在最后一步才暴露问题，根因却发生在更早的位置。检索阶段拿错资料，后续计划、工具参数和最终回复都可能保持内部一致，却建立在错误证据上。只给整条轨迹一个失败标签，团队无法判断该改检索、Prompt、工具还是完成条件。

首错归因按时间顺序找到第一个偏离预期的可观察事件：

1. 确认最终失败和外部证据；
2. 沿 Trace 向前检查状态、Observation、工具调用和授权；
3. 标记第一个足以改变后续结果的错误；
4. 区分根因、后续传播和未能拦截错误的控制缺口。

同一条失败可以同时记录 `root_cause_step`、`propagation_steps` 和 `missing_guardrail`。这样不会把下游 Agent 的错误总结当成根因，也不会因为找到首错就忽略本可提前阻断的运行时检查。

#### 用轨迹前缀缩短回归

端到端重放适合证明最终任务能够完成，但每次都从用户输入开始，成本高，也容易被前序随机性干扰。确认首错位置后，可以保存错误发生前的最小可重放状态，形成轨迹前缀用例：固定任务状态、必要 Context、外部工具结果和权限条件，只替换待验证的决策或组件。

轨迹前缀不能取代端到端回归。它用来快速验证局部修复，端到端用例负责检查真实链路中的状态构造、权限、工具和恢复是否仍然一致。评估集应同时保留二者，并通过同一个来源标识关联到原始生产失败。
...
Harness + Eval Engineering 把 Agent 质量从讨论对象变成工程对象。产品经理要定义发布问题、任务分布、能力上限、连续可靠性、证据标准、风险门禁和失败归因；工程团队负责让 Target 可运行、Trace 可读取、故障可注入、结果可复现。
...
6. 当前报告里的成功率是在衡量能力上限还是连续可靠性，重复运行条件是否支持这个结论？
