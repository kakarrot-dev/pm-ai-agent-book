...
团队常把 Tool、Skill、Workflow、Agent、Function Calling 和 MCP 放在同一张“能力清单”里。它们实际回答的是不同问题。

| 概念 | 它是什么 | 产品经理需要定义什么 | 它不是什么 |
| --- | --- | --- | --- |
| Tool | 读取或改变外部状态的原子动作 | 输入、权限、风险、返回、幂等与补偿 | 完整业务流程 |
| Skill | 可复用的任务方法，通常包含说明、约束、知识组织与工具使用规则 | 适用任务、输入输出、依赖、版本与评估 | 独立运行主体或天然安全边界 |
| Workflow | 由系统预先定义的步骤、状态与分支 | 顺序、条件、状态、超时、恢复与责任人 | 开放环境中的自主决策者 |
| Agent | 在边界内读取上下文、选择下一步并推进任务的运行主体 | 目标、权限、可用能力、预算、停止条件与完成证据 | 一个 Prompt 或一次模型调用 |
| Function / Tool Calling | 模型用结构化参数请求调用工具的机制 | Schema、校验、调用决定与错误处理 | 工具本身、执行权限或业务成功保证 |
| MCP | Host、Client 与 Server 之间发现和调用能力的开放协议 | Server 来源、能力范围、协议版本、授权、审批与信任边界 | Agent 框架、业务工作流或安全保证 |

在当前 MCP 规范中，Server 可暴露 Tools、Resources 和 Prompts：Tools 通常由模型选择，Resources 由应用选择，Prompts 由用户显式选择。它们的控制主体不同，不能都按“模型可自动调用的工具”处理。

一个常见组合是：Agent 根据任务选择一个 Skill；Skill 规定方法并引用若干 Tool；Workflow 约束必须遵守的关键步骤；部分 Tool 通过 MCP Server 被发现和调用；Function Calling 只负责把某次调用请求表达成结构化数据。连接成功只说明协议可用，不说明权限合理、结果可信或任务已经完成。

## 用模型能力卡代替“最强模型”

模型选择是一个多约束产品决策。通用排行榜无法回答模型在你的数据、工具、延迟和风险条件下是否合适。至少应在同一代表性评估集上比较以下维度。

| 维度 | 需要验证的问题 | 典型证据 |
| --- | --- | --- |
| 任务质量 | 核心任务、长尾任务和否决项表现怎样 | 分场景成功率、错误类型、人工评审 |
| 结构化输出与工具调用 | Schema 遵循、参数选择和失败恢复是否可靠 | 解析成功率、无效参数率、工具误用率 |
| 上下文与多模态 | 必要输入能否被稳定理解，长上下文是否真的带来收益 | 不同上下文长度和模态的对照评估 |
| 延迟与并发 | 端到端响应和峰值吞吐是否符合体验 | P50/P95 延迟、限流与排队数据 |
| 完整成本 | 成功任务而非单次调用的成本是否成立 | Token、缓存、重试、工具与人工成本 |
| 数据与合规 | 数据去向、保留、区域和审计是否满足要求 | 供应商条款、技术配置与合规评审 |
| 生命周期 | 版本升级、弃用和供应商故障能否应对 | 版本绑定、灰度、回滚与替代模型演练 |

模型能力卡必须绑定模型、提供方、具体版本、Prompt 与输出 Schema 版本，以及测试日期。只写一个营销名称，会让后续复现和回归失去基线。主模型与降级模型还要分别验证：降级后可以降低能力，但不能绕过权限、确认、证据和审计要求。
...
能力封装与连接：
- 是否需要 Skill，以及版本与评估：
- 是否使用 MCP，以及 Server 来源、协议版本、能力、授权与审批：
...
能力方案的起点是任务和产品责任，不是技术组件。模型处理语言和开放判断，上下文提供当前信息，记忆保存稳定偏好，知识库提供领域证据，工具执行动作，工作流约束关键步骤，多 Agent 隔离真正不同的职责。Tool、Skill、Workflow、Agent、Function Calling 和 MCP 位于不同层次，不能互相替代。

产品经理的责任是写清所需信息、允许判断、动作边界、失败降级和完成证据，让不同技术方案能够在同一需求下被比较和验证。模型选择也应以版本化能力卡和自有评估集为依据，而不是“最强模型”印象。
