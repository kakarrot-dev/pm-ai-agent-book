...
![路线图从证据出发](../diagrams/svg/book1-10-roadmap-and-collaboration-01.svg)

[查看 Mermaid 源码](../diagrams/source/book1-10-roadmap-and-collaboration-01.mmd)
...
大多数 Agent 产品应先改进任务定义、上下文、知识、工具、工作流和评估。训练会把行为写入参数，更新、解释和回滚都比修改外部系统更重。模型定制只有在以下条件同时接近成立时才值得进入路线图：
...
进入训练之前，还要判断缺口发生在哪一层。团队至少需要区分四种情况：

| 失败表现 | 优先方案 | 判断依据 |
| --- | --- | --- |
| 事实需要频繁更新、引用或按权限读取 | RAG 与知识治理 | 知识必须可追溯、可撤回，不能只存在模型参数中 |
| 模型缺少领域语言、知识或基础能力，多次采样仍几乎没有成功轨迹 | Mid-training | 需要在目标语料上继续训练，补齐模型能够理解和生成的基础分布 |
| 模型具备能力，但输出格式、风格或工具协议不稳定 | SFT | 已有高质量示范，可以明确写出理想输入与输出 |
| 模型偶尔能成功，任务结果可验证，但策略选择不稳定 | RL、拒绝采样或蒸馏 | 正确行为已出现，需要提高其概率，而不是从零补知识 |

`Pass@k` 可以帮助区分“不会做”和“会做但不稳定”。在代表性留出任务上多次采样，如果合理的 `k` 下仍几乎没有成功结果，RL 很难只靠终点奖励找到方向。缺领域知识或基础能力时评估 Mid-training；已有能力但不会按接口表达时先做 SFT；已经存在非零成功路径，并且环境能够可靠判断结果时，再评估 RL。

这张诊断表也有边界。一次实验不能直接证明失败来自模型参数，样本污染、工具不可用、Context 缺失和评估器错误都会制造相似现象。产品经理应要求团队给出失败分群、基线、数据来源和保留集结果，再决定训练路径。

若目标只是更新事实，优先使用知识库；若目标是快速调整规则，优先使用 Prompt、工作流或 Skill；若工具返回含糊，先修工具。训练不能替代产品和系统修复。
...
当电话、网页、规则核查和异常处理需要独立权限、可以并行等待并产生各自证据时，团队才评估多 Agent。若大量稳定场景仍受模型判断限制，并积累了高质量数据，再根据失败类型选择 Mid-training、SFT 或 RL。工具协议不稳定时继续修工具，不把接口问题写进模型参数。
...
模型定制只有在术语、格式或特定判断成为稳定瓶颈后考虑。领域语言和基础知识缺口可以评估 Mid-training，稳定格式与协议可以评估 SFT，已有非零成功路径的可验证判断可以评估 RL；知识时效和权限问题继续由治理系统解决。
...
![运营与业务专家](../diagrams/svg/book1-10-roadmap-and-collaboration-02.svg)

[查看 Mermaid 源码](../diagrams/source/book1-10-roadmap-and-collaboration-02.mmd)
...
[上一篇：上线、运营与持续迭代](09-launch-operations-and-iteration.md) · [篇章目录](README.md) · [下一篇：后记](11-afterword.md)
